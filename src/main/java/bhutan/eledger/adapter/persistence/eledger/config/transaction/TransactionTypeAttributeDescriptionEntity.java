...
@Entity
@Table(name = "transaction_type_attribute_description", schema = "eledger_config")
@NoArgsConstructor
class TransactionTypeAttributeDescriptionEntity extends TranslationEntity {
    @Id
    @SequenceGenerator(name = "transaction_type_attribute_description_id_seq", schema = "eledger_config", sequenceName = "transaction_type_attribute_description_id_seq", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "transaction_type_attribute_description_id_seq")
    @Column(name = "id")
    private Long id;
