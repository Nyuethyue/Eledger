...
CREATE OR REPLACE FUNCTION eledger.fn_get_payment_advice_data(p_tpn character varying, p_calculation_date date, p_drn character varying DEFAULT NULL::character varying)
 RETURNS TABLE(tpn character varying, tp_name character varying, drn character varying, period_year character varying, period_segment character varying, deadline date, payable_lines json)
 LANGUAGE plpgsql
AS $function$
                 ...
                 FROM (
                          ...
                          FROM (
                                   ...
                                   FROM eledger.el_taxpayer tp
                                            ...
                                            inner join eledger_config.el_transaction_type tt on tt.id = t.transaction_type_id
                                   WHERE tp.tpn = p_tpn
                                     AND a.account_type = 'A'
                                     and tt.code <> 'REFUND_FROM_NET_NEGATIVE'
