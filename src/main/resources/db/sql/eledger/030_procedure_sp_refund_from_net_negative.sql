CREATE OR REPLACE PROCEDURE eledger.sp_refund_from_net_negative(p_taxpayer_id bigint, p_calculation_date date)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    r_pay            record;
...
BEGIN
    ...
    SELECT id INTO v_action_type_id FROM eledger_config.el_accounting_action_type WHERE name = 'NET_NEGATIVE_TO_TAXPAYER_ACCOUNT';
    ...
    WHERE tc.accounting_action_type_id = v_action_type_id
      AND eledger.fn_get_attribute_value(l.transaction_id, 'target_transaction_id')::bigint = p.transaction_id
