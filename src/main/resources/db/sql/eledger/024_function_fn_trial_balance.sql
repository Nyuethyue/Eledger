CREATE OR REPLACE FUNCTION eledger.fn_trial_balance(p_tpn character varying, p_language_code character varying,
                                                    p_tax_type_code character varying, p_year character varying,
                                                    p_segment character varying, p_start_date date, p_end_date date,
                                                    p_offset bigint, p_limit bigint)
    RETURNS TABLE
            (
...
BEGIN
    RETURN QUERY
        ...
        FROM (
                 ...
                 WHERE tp.tpn = p_tpn
                   AND a.transaction_date >= coalesce(p_start_date, a.transaction_date)
                   AND a.transaction_date <= coalesce(p_end_date, a.transaction_date)
